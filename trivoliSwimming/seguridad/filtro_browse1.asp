...
function filtrar()
{
  ...
  if (sel.substr(0,1) == "R") // Restaurar
    {
      window.opener.ifrm.location = '<%= l_destino %>&orden=<%= l_orden %>&filtro='; 
	  window.close();  
    }
  else
    {
	...
	{
	  window.location = "filtro_browse_txt1.asp?campo=" + sel.substr(1) + "&fuente=<%= l_destino %>&orden=<%= l_orden %>"
	}  
	else
 	  if (sel.substr(0,1) == "N") // Es numerico 
  	    window.location = "filtro_browse_num1.asp?campo=" + sel.substr(1) + "&fuente=<%= l_destino %>&orden=<%= l_orden %>"
	  else	// Es fecha
  	    window.location = "filtro_browse_fec1.asp?campo=" + sel.substr(1) + "&fuente=<%= l_destino %>&orden=<%= l_orden %>"
    }
}
