...
'-Obtengo-el-código-del-Lugar----------------------------------
sql = " SELECT lugcod, lugdes FROM tkt_config "
sql = sql & " inner join tkt_lugar on tkt_lugar.lugnro = tkt_config.lugnro "
rsOpen myRs, cn, sql, 0
if not myRs.eof then
'	Session("Titulo") = "" & ucase(myRs("lugcod")) & " - "
	Session("Titulo") = "" & ucase(myRs("lugdes")) & " - "
else
	Session("Titulo") = "ERROR"
end if
myRs.Close
'---------------------------------------------------------------
