...
l_nombre = request.Form("nombre")
	...
	if l_tipo = "A" then 
		l_sql = "INSERT INTO medicos_derivadores"
		l_sql = l_sql & " (nombre,empnro,created_by,creation_date,last_updated_by,last_update_date)"
		l_sql = l_sql & " VALUES (" 
		l_sql = l_sql &  "'" & l_nombre & "','" & session("empnro") & "','"&session("loguinUser")&"',GETDATE(),'"&session("loguinUser")&"',GETDATE())"
		
	else
		l_sql = "UPDATE medicos_derivadores"
