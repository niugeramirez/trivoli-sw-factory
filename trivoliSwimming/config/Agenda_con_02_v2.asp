...
'response.write "l_fechadesde" & l_fechadesde
'response.write "l_fechahasta" & l_fechahasta
'response.write "l_filtro" & l_filtro
'
'response.write "l_hd" & l_hd
'response.write "l_md" & l_md
'response.write "l_hh" & l_hh
'response.write "l_mh" & l_mh
...
'response.write cdate(l_fechaaux)
  ...
  do until l_rs2.eof
	...
	<%
	  ...
	  do until l_rs3.eof
			...
			if l_rs3("horainicio") >= l_horadesde  and l_rs3("horainicio") <= l_horahasta then
			%>	
	    
	        	<a href="Javascript:abrirVentana('Asignarpacientes_con_02.asp?Agenda=S&Tipo=A&cabnro=<%= l_rs3("id") %>' ,'',600,350);"><%'= cambiafecha(l_rs3("Fecha"),"","")%><%= l_rs3("horainicio")%>&nbsp;<%'= weekday(cdate(l_rs2("Fecha"))) %></a>
