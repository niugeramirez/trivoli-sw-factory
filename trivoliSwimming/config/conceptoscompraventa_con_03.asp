...
'response.write "l_id"&l_id & "<br>"
	...
	if l_tipo = "A" then 
		l_sql = "INSERT INTO conceptosCompraVenta  "
		' Multiempresa
		...
		l_sql = l_sql & " VALUES ('" & l_descripcion & "','" & session("empnro") &"','"&session("loguinUser")&"',GETDATE(),'"&session("loguinUser")&"',GETDATE())"
		
	else
		l_sql = "UPDATE conceptosCompraVenta "
