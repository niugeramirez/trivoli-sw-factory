...
%>
    <tr>
		...
		<th width="100">Monto Compra</th>
        <th width="200">Pagado</th>			
		<th width="200">Saldo</th>					
...
case 6 'Obligaciones a pagar

	
	l_sql = " select sum(tab_agrup.monto_compra)	as monto_compra "
	l_sql = l_sql & " 		,sum(tab_agrup.pagado)		as pagado	 "
	l_sql = l_sql & " 		,ISNULL(sum(tab_agrup.monto_compra),0)	- ISNULL(sum(tab_agrup.pagado),0) as saldo "
	l_sql = l_sql & " 		,nombre_proveedor "
	l_sql = l_sql & " from	( "
	...
	l_sql = l_sql & " 			where  compras.fecha >= "& cambiafecha(l_fechadesde,"YMD",true)
	l_sql = l_sql & " 			AND compras.fecha <= "& cambiafecha(l_fechahasta ,"YMD",true)
	l_sql = l_sql & " 			AND compras.empnro =  " & Session("empnro") 
	l_sql = l_sql & " 		)		tab_agrup "
	l_sql = l_sql & " group by nombre_proveedor	 "
	l_sql = l_sql & " having ISNULL(sum(tab_agrup.monto_compra),0)	- ISNULL(sum(tab_agrup.pagado),0)	<> 0  "
	l_sql = l_sql & " order by saldo "
...
<table>
	...
	<tr>
        <td  colspan="6" align="center" ><h3>Obligaciones a pagar:&nbsp;<%= l_fechadesde %>&nbsp; al <%= l_fechahasta %>&nbsp;&nbsp;</h3></td>	
    </tr>
...
<% 
    ...
    do while not l_rs.eof	
		...
		arrData(i,1) = l_rs("nombre_proveedor")
		arrData(i,2) = round( l_rs("saldo"),0)
%>	
	    <tr>
			
	        <td align="center"><%= l_rs("nombre_proveedor") %></td>
...
<% 
	...
	strXML = "<graph xaxisname='Proveedores' yaxisname='Saldo' hovercapbg='DEDEBE' hovercapborder='889E6D' rotateNames='0' animation='1' yAxisMaxValue='0' numdivlines='9' divLineColor='CCCCCC' divLineAlpha='80' decimalPrecision='0' showAlternateVGridColor='1' AlternateVGridAlpha='30' AlternateVGridColor='CCCCCC' caption='Obbligaciones a Pagar' subcaption='' > "
   ...
   strXML = strXML & strXML_C14
   strXML = strXML & "</categories> "
   strXML = strXML & " <dataset seriesname='Saldo' color='FDC12E' alpha='70'> " 
     strXML = strXML & strXML_1P4
   strXML = strXML & " </dataset> "
   'strXML = strXML & " <dataset seriesname='Salidas' color='8E468E' showValues='1' alpha='70'> "
   strXML = strXML & strXML_R14
...
case 7 'Estado cheques
	l_sql =  " select	cheques_estado.id		as idestado_cheque "
	l_sql = l_sql & " 		,cheques_estado.estado	as	estado_cheque "
	l_sql = l_sql & " 		,sum(importe)			as	monto  "
	l_sql = l_sql & " 		,count(*)				as	cantidad "
	l_sql = l_sql & " FROM cheques  "
	l_sql = l_sql & " inner join cheques_estado on cheques_estado.id = dbo.get_estado_cheque(cheques.id) "
	l_sql = l_sql & " where  cheques.fecha_emision >= "& cambiafecha(l_fechadesde,"YMD",true)
	l_sql = l_sql & " AND cheques.fecha_emision <= "& cambiafecha(l_fechahasta ,"YMD",true)
	l_sql = l_sql & " AND cheques.empnro =  " & Session("empnro") 
	l_sql = l_sql & " group by	cheques_estado.id "
	l_sql = l_sql & " 			,cheques_estado.estado "
...
<table>
	...
	<tr>
        <td  colspan="6" align="center" ><h3>Estado Cheques emitidos:&nbsp;<%= l_fechadesde %>&nbsp; al <%= l_fechahasta %>&nbsp;&nbsp;</h3></td>	
    </tr>
...
<% 
    ...
    do while not l_rs.eof	
		...
		arrData(i,1) = l_rs("estado_cheque")
		arrData(i,2) = round(l_rs("monto"),0)
		arrData(i,3) = l_rs("cantidad")
%>	
	    <tr>
			
	        <td align="center"><%= l_rs("estado_cheque") %></td>
			<td align="center"><%= l_rs("monto") %></td>
	        <td align="center"><%= l_rs("cantidad") %></td>	
...
<% 
	...
	strXML = "<graph xaxisname='Estado' yaxisname='Monto' hovercapbg='DEDEBE' hovercapborder='889E6D' rotateNames='0' animation='1' yAxisMaxValue='0' numdivlines='9' divLineColor='CCCCCC' divLineAlpha='80' decimalPrecision='0' showAlternateVGridColor='1' AlternateVGridAlpha='30' AlternateVGridColor='CCCCCC' caption='Estado Cheques' subcaption='' > "
   ...
   strXML = strXML & strXML_C14
   strXML = strXML & "</categories> "
   strXML = strXML & " <dataset seriesname='Monto' color='FDC12E' alpha='70'> " 
     strXML = strXML & strXML_1P4
   strXML = strXML & " </dataset> "
   'strXML = strXML & " <dataset seriesname='Salidas' color='8E468E' showValues='1' alpha='70'> "
   strXML = strXML & strXML_R14
...
case 8 'Obligaciones a cobrar

	l_sql = " select sum(tab_agrup.monto_venta)	as monto_venta "
	l_sql = l_sql & " 		,sum(tab_agrup.cobrado)		as cobrado	 "
	l_sql = l_sql & " 		,ISNULL(sum(tab_agrup.monto_venta),0)	- ISNULL(sum(tab_agrup.cobrado),0)	as	saldo 	 "
	l_sql = l_sql & " 		,cliente_nombre "
	l_sql = l_sql & " from	( "
	...
	l_sql = l_sql & " 			where  ventas.fecha >= " & cambiafecha(l_fechadesde,"YMD",true)
	l_sql = l_sql & " 			AND ventas.fecha <=  "& cambiafecha(l_fechahasta,true)
	l_sql = l_sql & " 			AND ventas.empnro = "& Session("empnro")
	l_sql = l_sql & " 		)		tab_agrup  "
	l_sql = l_sql & " group by cliente_nombre  "
	l_sql = l_sql & " having ISNULL(sum(tab_agrup.monto_venta),0)	- ISNULL(sum(tab_agrup.cobrado),0) <> 0 "
	l_sql = l_sql & " order by ABS(ISNULL(sum(tab_agrup.monto_venta),0)	- ISNULL(sum(tab_agrup.cobrado),0)) desc "
...
<table>
	...
	<tr>
        <td  colspan="6" align="center" ><h3>Obligaciones a Cobrar:&nbsp;<%= l_fechadesde %>&nbsp; al <%= l_fechahasta %>&nbsp;&nbsp;</h3></td>	
    </tr>
...
<% 
    ...
    do while not l_rs.eof	
		...
		arrData(i,1) = l_rs("cliente_nombre")
		arrData(i,2) = round( l_rs("saldo"),0)
%>	
	    <tr>
			
	        <td align="center"><%= l_rs("cliente_nombre") %></td>
			<td align="center"><%= l_rs("monto_venta") %></td>
	        <td align="center"><%= l_rs("cobrado") %></td>	
			<td align="center"><%= l_rs("saldo")%></td>
...
<% 
	...
	strXML = "<graph xaxisname='Clientes' yaxisname='Saldo' hovercapbg='DEDEBE' hovercapborder='889E6D' rotateNames='0' animation='1' yAxisMaxValue='0' numdivlines='9' divLineColor='CCCCCC' divLineAlpha='80' decimalPrecision='0' showAlternateVGridColor='1' AlternateVGridAlpha='30' AlternateVGridColor='CCCCCC' caption='Obbligaciones a Cobrar' subcaption='' > "
