...
Fecha		: 25/03/2004
Modificado	:
	21/10/2005 - Fapitalle N. - se reemplazo la sql por la nueva relacion entre cap_calendario y cap_evento
-----------------------------------------------------------------------------
-->
<% 
on error goto 0
...
'Me fijo si el empleado ya existe
	...
	if l_rs.eof then
		...
		if l_tipo = "P" then
			...
			l_sql = l_sql & " INNER JOIN cap_eventomodulo ON cap_calendario.evmonro = cap_eventomodulo.evmonro "
			l_sql = l_sql & " WHERE cap_eventomodulo.evmonro = " & l_evenro 
			rsOpen l_rs, cn, l_sql, 0
			if not l_rs.eof then
			   l_rs.MoveFirst
			end if
			Do While Not l_rs.eof
				l_sql = "INSERT INTO cap_partcal "
...
'l_rs.close
	...
	l_sql = l_sql & " INNER JOIN cap_calendario ON cap_calendario.evmonro = cap_eventomodulo.evmonro "
...
<html>
	...
	<body onload="JavaScript: validar()">
		<form action="../cgi-bin/postmail.exe" method="post" name="FormVar" id="FormVar">
			<input type="Hidden" name="Host" value="<%= cEmailHost %>">
			<input type="Hidden" name="Port" value="<%= cEmailPort %>">
			<input type="Hidden" name="Userid" value="<%= cEmailUserid %>">
			<input type="Hidden" name="Date" value="<%= FormatDateTime(now,2) %>">
			<input type="Hidden" name="FromAddress" value="<%= cEmailFromAddress %>">
			<input type="Hidden" name="FromName" value="<%= cEmailFromName %>">
			<input type="Hidden" name="ReplyTo" value="<%= cEmailReplyTo %>">
			<input type="Hidden" name="ToAddress" value="<%= l_empemail %>">
		<% select case l_tipo %>
				<%	case "C" %>
			<input type="Hidden" name="Subject" value="Alta a eventos de capacitación">
			<input type="Hidden" name="Body" value="El funcionario <%= l_empleado  %> se ingreso como Candidato a un pedido del evento de <%= l_evedesabr %> de capacitación en el Módulo de Autogestión.">
				<% 	case "P" %>
			<input type="Hidden" name="Subject" value="Alta a eventos de capacitación">
			<input type="Hidden" name="Body" value="El funcionario <%= l_empleado  %> se ingreso como Participante a un pedido del evento de <%= l_evedesabr %> de capacitación en el Módulo de Autogestión.">
				<% 	case "Q" %>
			<input type="Hidden" name="Subject" value="Baja a eventos de capacitación">
			<input type="Hidden" name="Body" value="El funcionario <%= l_empleado  %> se dio de Baja a un pedido del evento de <%= l_evedesabr %> de capacitación en el Módulo de Autogestión.">
		<% end select %>
		</form>
	</body>
<script>
