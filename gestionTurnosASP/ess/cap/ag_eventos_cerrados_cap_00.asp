...
<!--
-----------------------------------------------------------------------------
Archivo		: ag_eventos_cerrados_cap_00.asp
...
<title>Participación en cursos cerrados- Capacitación - RHPro &reg;</title>
...
function llamadaexcel(){ 
	abrirVentana("ag_eventos_cerrados_cap_excel.asp?ternro=<%= l_ternro%>&orden="+document.ifrm.datos.orden.value+"&filtro="+escape(document.ifrm.datos.filtro.value),'execl',50,50);
}
...
function candidato(){
	if (document.ifrm.datos.cabnro.value == 0){
		...
		//alert("ok");
		abrirVentanaH('ag_eventos_cerrados_cap_03.asp?ternro=<%= l_ternro %>&evenro=' + document.ifrm.datos.cabnro.value + '&tipo=C','','','');
	...
	if (document.ifrm.datos.cabnro.value == 0){
		alert("Debe seleccionar un Evento.");
		 ...
		 if ((ifrm.jsSelRow.cells(4).innerText.slice(0) < 1)) {
			...
			abrirVentanaH('ag_eventos_cerrados_cap_03.asp?ternro=<%= l_ternro %>&evenro=' + document.ifrm.datos.cabnro.value + '&tipo=P','','','');
...
function quitar(){
	if (document.ifrm2.datos.cabnro.value == 0){
		...
		//alert("ok");
		abrirVentanaH('ag_eventos_cerrados_cap_03.asp?ternro=<%= l_ternro %>&evenro=' + document.ifrm2.datos.cabnro.value + '&tipo=Q','','400','150');
	}
}
...
<tr>
	<td colspan="2" height="100%">
		<table border="0" cellpadding="0" cellspacing="0"  height="100%">
			<tr>
				<td class="th2">Eventos Cerrados en los que está inscripto</td>
				<td class="th2" align="right">
					<% 'call Mostrarboton ("sidebtnABM", "#","Quiter del Evento","MenuPopUp('elMenu1',event)","MenuPopDown('elMenu1')") %>
					<% 'call MostrarBoton ("sidebtnABM", "Javascript:quitar();","Quitar del Evento")%>
					<a class=sidebtnABM href="Javascript:Javascript:quitar();">Quitar del Evento</a>					
				</td>
			</tr>
	        <tr valign="top">
	        	<td align="left" style="" height="50%" nowrap  colspan="2">
   	  				<iframe  scrolling="Yes" name="ifrm2" src="ag_eventos_cerrados_cap_01.asp?ternro=<%= l_ternro %>" width="100%" height="100%"></iframe> 
   				</td>        				
   			</tr>
			<tr>
				<td class="th2">Listado de Eventos Cerrados</td>
				<td class="th2" align="right">
					<% 'call MostrarOpcionMenu ("sidebtnABM", "#","Incorporar al Evento","MenuPopUp('elMenu1',event)","MenuPopDown('elMenu1')") %>
					<a class=sidebtnABM href="#" onclick="MenuPopUp('elMenu1',event)" onmouseout="MenuPopDown('elMenu1')" >Incorporar al Evento</a><br>										
				</td>
			</tr>
	        <tr valign="top">
	        	<td align="left" style="" height="50%" colspan="2">
   	  				<iframe  scrolling="Yes" name="ifrm" src="ag_eventos_cerrados_cap_02.asp?ternro=<%= l_ternro %>" width="100%" height="100%"></iframe> 
   				</td>        				
   			</tr>
		</table>
	</td>
