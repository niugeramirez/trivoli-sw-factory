...
<%
'================================================================================
'Archivo		: carga_calificomp_eva_00.asp
'Descripción	: Cargar resultado gral de objetivos RDP
'Autor			: 15-02-2005
'Fecha			: Leticia Amadio
'Modificado		: 09-05-2005 - LA - terminarla . 
...
'XXXXXXXX  if l_habCalifArea="NO" then

	if l_cantproysaprob <= 0 or l_evatevnro <> cint(cconsejero)  then 	'evatevnro
		areaResu= areaResu & "<select name=areatrnro"& l_rs("evatitnro")&" disabled style="" width:120 "">"
	else 
		areaResu= areaResu & "<select name=areatrnro"& l_rs("evatitnro")&" style="" width:120 "">"
	end if
...
if l_cantproysaprob <= 0 or  l_evatevnro <> cint(cconsejero) then 
...
Set l_rs1 = Server.CreateObject("ADODB.RecordSet") 
l_sql = "SELECT  evaresu.evatrnro, evaresu.evaresudes, evatipresu.evatrvalor, evatipresu.evatrdesabr"
l_sql = l_sql & " FROM evaresu "
...
if l_cantproysaprob <= 0 or  l_evatevnro <> evatevnro then
	compResu = compResu & "<select name=evatrnro"&evafacnro&evatevnro&" style="" width:100 "" disabled>"
else
	compResu = compResu & "<select name=evatrnro"&evafacnro&evatevnro&" style="" width:100 "">"
end if
...
<iframe src="blanc.asp"  name="terminarsecc" src="termsecc_areasyresultados_eva_00.asp?evacabnro=<%=l_evacabnro%>&evaseccnro=<%=l_evaseccnro%>&evldrnro=<%=l_evldrnro%>&evatevnro=<%=l_evatevnro%>" style="visibility:hidden;width:0;height:0">
</iframe>
