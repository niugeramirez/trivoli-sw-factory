...
'l_sql = l_sql & "		 AND evasecc.evaoblig= -1"
'l_sql = l_sql & " INNER JOIN evaoblieva ON evaoblieva.evaseccnro= evadetevldor.evaseccnro"
'l_sql = l_sql & "		AND  evaoblieva.evatevnro= evadetevldor.evatevnro"
...
'l_sql = l_sql & " WHERE evadetevldor.evacabnro = " & l_evacabnro
'l_sql = l_sql & " AND   evaoblieva.evatevobli = -1" 
'l_sql = l_sql & " AND   evadetevldor.evldorcargada = 0" 
'rsOpen l_rs, cn, l_sql, 0
'if l_rs.eof then
 ' 	l_readonly = ""
	'l_cabaprobada =""
    ...
    <tr>
		<td width="70%" align="left">
        	...
        	<%if trim(l_depende)<>"" then
				...
				Para esta Secci&oacute;n: 
        		(Debe terminar <%=l_depende%>)
        	<%end if%>
        </td>
        <td align="right">&nbsp;
		<!--
		<b>Evaluaci&oacute;n Aprobada </b>&nbsp;
			<input <%'if l_supervisor <> l_evaluador then%>readonly disabled <%'else%><%'=l_readonly%> <%'end if%> name="cabaprobada" type="Checkbox" <%'= l_cabaprobada%> onclick="ActualizarCab();"> 
			&nbsp;
			-->
        </td>
    </tr>
