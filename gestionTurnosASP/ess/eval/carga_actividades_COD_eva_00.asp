...
<% 
'=====================================================================================
'Archivo  : carga_atividades_COD_eva_00.asp
'Objetivo : ABM de actividades or objetivos (mas de una)
'Fecha	  : 03-02-2005
'Autor	  : CCRossi
'Modificacion: 16-02-2005 CRossi cambiar osicion columna fecha
...
<table>
    <tr style="border-color :CadetBlue;">
        ...
        <th class="th2">Apoyo Comprometido</th>
        <th class="th2">&nbsp;</th>
    </tr>
...
l_sql = l_sql & " ORDER BY evaobjetivo.evatipobjnro, evatipobjdabr, evaplan.evaobjnro, evaplnro, aspectomejorar, planaccion, planfecharev, evaobjdext  "
rsOpen l_rs, cn, l_sql, 0
...
%>
    <tr>
       ...
       <td valign=top>
			<a href=# onclick=" if (validarfecha(document.datos.planfecharev<%=l_rs("evaplnro")%>)) { if (Controlar(document.datos.aspectomejorar<%=l_rs("evaplnro")%>) && Controlar(document.datos.planaccion<%=l_rs("evaplnro")%>) && Vacio(document.datos.aspectomejorar<%=l_rs("evaplnro")%>)) { grabar.location='grabar_plan_accion_00.asp?tipo=M&evldrnro=<%=l_evldrnro%>&evaplnro=<%=l_rs("evaplnro")%>&aspectomejorar='+escape(document.datos.aspectomejorar<%=l_rs("evaplnro")%>.value)+'&planaccion='+escape(document.datos.planaccion<%=l_rs("evaplnro")%>.value)+'&planfecharev='+document.datos.planfecharev<%=l_rs("evaplnro")%>.value;document.datos.grabado<%=l_rs("evaplnro")%>.value='M';}}">Grabar</a>
			<br>
			<input type="text" class="rev" style="background : #e0e0de;" readonly disabled name="grabado<%=l_rs("evaplnro")%>" size="1">
			<br>
			<a href=# style="color:red;" onclick="if (confirm('¿ Desea Eliminar la Actividad?')==true) { grabar.location='grabar_plan_accion_00.asp?tipo=B&evaplnro=<%=l_rs("evaplnro")%>&evldrnro=<%=l_evldrnro%>&aspectomejorar='+document.datos.aspectomejorar<%=l_rs("evaplnro")%>.value+'&planaccion='+document.datos.planaccion<%=l_rs("evaplnro")%>.value+'&planfecharev='+document.datos.planfecharev<%=l_rs("evaplnro")%>.value};document.datos.grabado<%=l_rs("evaplnro")%>.value='B';">Eliminar <br>Actividad</a>
		</td>
    </tr>
<%
	l_rs.MoveNext
...
%>
    <tr>
    	<td align=left>
			<%' BUSCAR objetivos
			Set l_rs = Server.CreateObject("ADODB.RecordSet")
			l_sql = "SELECT DISTINCT evaobjetivo.evaobjnro,evaobjetivo.evaobjdext "
			...
			'response.write l_sql
			rsOpen l_rs, cn, l_sql, 0 %>
			<select name="evaobjnro">
			<%do while not l_rs.eof%>
				<option value=<%=l_rs("evaobjnro")%>><%=left(l_rs("evaobjdext"),33)%>...</option>
			<%l_rs.MoveNext
			loop
			l_rs.Close
			set l_rs = nothing%>
		...
		<td valign=top>
			<a href=# onclick=" if (validarfecha(document.datos.planfecharev)) { if (Controlar(document.datos.aspectomejorar) ) {grabar.location='grabar_plan_accion_00.asp?tipo=A&evldrnro=<%=l_evldrnro%>&aspectomejorar='+escape(document.datos.aspectomejorar.value)+'&planaccion='+escape(document.datos.planaccion.value)+'&planfecharev='+document.datos.planfecharev.value+'&evaobjnro='+document.datos.evaobjnro.value;document.datos.grabado.value='G';}}">Grabar</a>
			<br>
