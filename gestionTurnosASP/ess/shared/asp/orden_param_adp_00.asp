...
if len(l_filtro) <> 0 then
	if left(l_filtro,1) = "'" then
		l_filtro = mid(l_filtro,2,len(l_filtro))
		l_filtro = left(l_filtro, len(l_filtro) - 1  )
	end if		
end if
...
<!-- <link href="/serviciolocal/ess/<%=c_estilo %>" rel="StyleSheet" type="text/css"> -->
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
<title>Orden - Administraci&oacute;n de Personal - RHPro &reg;</title>
</head>
<script>
function valor()
{
...
window.resizeTo(360,<%= l_cantidad * 25 + 120 %>)
</script>
...
<table cellspacing="1" cellpadding="0" border="0" width="100%">
  <tr>
    <th class="th2" colspan="3">Orden</th>
  </tr>
  <tr>
    <td class="th2">&nbsp;</td>
    <td class="th2" align="center"><br>Ascendente</td>
    <td class="th2" align="center"><br>Descendente</td>
  </tr>
<%
l_i = 1
...
<tr>
	...
	<td align="center"><input type="Radio" name="orden" value="<%= l_listaCampos(l_i) %>" <% If l_i = 1 then%>checked<% End If %>></td>
	<td align="center"><input type="Radio" name="orden" value="-<%= l_listaCampos(l_i) %>"></td>
</tr>
<%
  l_i = l_i + 1
loop
...
<tr>
    <td align="right">
		<br>
		<a class=sidebtnABM href="Javascript:window.opener.ifrm.location='<%=l_destino %>?orden=' + valor() + '&filtro=' + escape(document.datos.filtro.value) +'&'+opener.param(); window.close();">Aceptar</a>
