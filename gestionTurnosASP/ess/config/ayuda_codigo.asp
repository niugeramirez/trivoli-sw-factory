...
<title><%= Session("Titulo")%>Ayuda - RHPro &reg;</title>
...
%>
<table border=0>
<form action="ayuda_codigo.asp" method="get" name="f1">
...
if l_des = 0 then
	l_sql = "SELECT min(cast(" & l_campocod & " as int )) FROM " & l_tabla
...
if l_has = 0 then
	l_sql = "SELECT max(cast(" & l_campocod & " as int )) FROM " & l_tabla
	rsOpen l_rs, cn, l_sql, 0 
	l_ultimo = l_rs(0)
	l_rs.close
else
...
<table centered width=100%>
    ...
    IF request("Acepta") <> "" THEN
	  ...
	  l_sql = l_sql & " AND (" & l_des & " = 0 OR " & l_campocod & " >= " & l_des & ")"
	  l_sql = l_sql & " AND (" & l_has & " = 0 OR " & l_campocod & " <= " & l_has & ")"
	  if l_descrip <> " " then l_sql = l_sql & " AND ("&l_campodest&" LIKE '" & l_descrip & "%" & "')" end if
	  'l_sql = l_sql & " ORDER BY " &  l_campocod
	  l_sql = l_sql & " ORDER BY " &  l_campodest
