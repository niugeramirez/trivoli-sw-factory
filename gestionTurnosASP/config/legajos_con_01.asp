...
if l_orden = "" then
  l_orden = " ORDER BY legape, legnom "
end if
...
%>
<tr>
	 <td colspan="7" >No existen Legajos cargados para el filtro ingresado.</td>
</tr>
<%else
    l_primero = l_rs("legnro")
	l_cant = 0
	do until l_rs.eof
		l_cant = l_cant + 1
	%>
	    <tr ondblclick="Javascript:parent.abrirVentana('legajos_con_02.asp?Tipo=M&cabnro=' + datos.cabnro.value,'',800,500)" onclick="Javascript:Seleccionar(this,<%= l_rs("legnro")%>)">
	        ...
	        <td width="10%" nowrap><%= l_rs("legape")%></td>
	        <td width="10%" nowrap><%= l_rs("legnom")%></td>						
	        <td width="10%" nowrap><%= l_rs("legdni")%></td>			
	        <td width="10%" nowrap><%= l_rs("legdom")%></td>					
