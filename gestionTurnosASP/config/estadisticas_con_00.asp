...
function Actualizar(destino){
	...
	if (destino== "exel")
    	abrirVentana("estadisticas_con_01.asp?" + param + "&excel=true",'execl',250,150);
	else
		document.ifrm.location = "estadisticas_con_01.asp?" + param;			
...
'		l_fecini = "01/0" & month(date()) - 1 & "/" & year(date())
'		l_fecfin = cdate("01/0" & month(date()) & "/" & year(date())) - 1
'	else
'		l_fecini = "01/" & month(date()) - 1 & "/" & year(date())
