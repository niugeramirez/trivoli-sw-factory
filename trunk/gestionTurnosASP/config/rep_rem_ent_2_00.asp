...
l_sitnro2 	  = request.querystring("qsitnro")
...
function Actualizar(destino){
	...
	if (document.datos.sitnro.value == 0) {
  		alert("Debe Seleccionar un Sitio");
  		document.datos.sitnro.focus();
		return;
	}		
	
	param = "qfecini=" + document.all.fecini.value + "&qfecfin=" + document.all.fecfin.value + "&qsitnro=" + document.all.sitnro.value;
	
	if (destino== "exel")
    	abrirVentana("rep_exp_buques_con_01.asp?" + param + "&excel=true",'execl',250,150);
	else
		document.ifrm.location = "rep_rem_ent_2_01.asp?" + param;			
...
<% End If %>
<form name="datos">
<input  type="hidden" name="sitnro2" value="<%= l_sitnro2 %>" >
<table border="0" cellpadding="0" cellspacing="0" height="100%">
	...
	<tr>
		...
		<td><select name="sitnro" size="1" style="width:120;">
			...
			<script>document.datos.sitnro.value= document.datos.sitnro2.value </script>
			</td>	
	</tr>
