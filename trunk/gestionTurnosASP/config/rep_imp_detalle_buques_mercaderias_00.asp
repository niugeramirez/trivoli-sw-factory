...
function Actualizar(destino){
	...
	param = "qfecini=" + document.all.fecini.value + "&qfecfin=" + document.all.fecfin.value + "&repnro=" + document.all.repnro.value;
	
	if (destino== "exel")
    	abrirVentana("rep_exp_buques_con_01.asp?" + param + "&excel=true",'execl',250,150);
	else
		document.ifrm.location = "rep_imp_detalle_buques_01.asp?" + param;			
...
if l_fecini = "" then
	...
	if month(date()) < 10 then
		l_fecini = "01/0" & month(date()) & "/" & year(date())
		l_fecfin = "31/0" & month(date()) & "/" & year(date())
	else
		l_fecini = "01/" & month(date()) & "/" & year(date())
