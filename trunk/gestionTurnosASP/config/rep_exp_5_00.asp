...
l_expnro2 	  = request.querystring("qexpnro")
...
function Actualizar(destino){
	...
	if (document.datos.expnro.value == 0) {
  		alert("Debe Seleccionar una Exportadora");
  		document.datos.expnro.focus();
		return;
	}					
	
	param = "qfecini=" + document.all.fecini.value + "&qfecfin=" + document.all.fecfin.value + "&qexpnro=" + document.all.expnro.value;
	
	if (destino== "exel")
    	abrirVentana("rep_exp_buques_con_01.asp?" + param + "&excel=true",'execl',250,150);
	else
		document.ifrm.location = "rep_exp_5_01.asp?" + param;			
...
<% End If %>
<form name="datos">
<input  type="hidden" name="expnro2" value="<%= l_expnro2 %>" >
<table border="0" cellpadding="0" cellspacing="0" height="100%">
	...
	<tr>
		...
		<td><select name="expnro" size="1" style="width:120;">
			...
			<script>document.datos.expnro.value= document.datos.expnro2.value </script>
			</td>	
	</tr>
