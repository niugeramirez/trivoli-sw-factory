...
' de base de datos
  ...
  const max_horas = 8 'cantidad de hs permitidad para un dia o una semana cuando el timpo de novedad es 1 (para red Megatoner)
...
if l_caso1 or l_caso2 or l_caso3 then
	'busco el nombre del empleado.
	Set l_rs6 = Server.CreateObject("ADODB.RecordSet")
	l_sql6 = "SELECT empleg, terape, ternom  FROM empleado WHERE empleado.ternro = " & l_gnovotoa
	l_rs6.open l_sql6,cn
	if not l_rs6.eof then 
		l_empleado = l_rs6("empleg") & "-" & l_rs6("terape") & ", " & l_rs6("ternom")
	end if
end if

if l_caso1 then 
 response.write "<script>alert(' El empleado " &  l_empleado & " tiene un día con más de " &  max_horas  & " hs.   ');history.back();</script> "
end if
if l_caso2 then 
 response.write "<script>alert(' El empleado " & l_empleado &  " acumula más de " & max_horas & " hs en la semana.  ');history.back();</script>"
end if
if l_caso3 then 
 response.write "<script>alert(' El empleado " & l_empleado & " tiene otra novedad que junto con ésta suman mas de " & max_horas & " hs. en una semana. ');history.back();</script>" 
end if
...
' tomar parametros
  ...
  l_grabar		= request.Form("listempleados")
...
' buscar ternro  -------------------------------------------------	
  ...
  l_ubicacion	= InStr(l_grabar,",")
  l_ubicacion	= l_ubicacion - 1
  l_ternro		= left(l_grabar,l_ubicacion)
		...
		if l_gtnovnro = 1 then 
			...
			if not l_rs5.eof then 'tiene puesto
			    if dia(l_gnovdesde) >= "01" and dia(l_gnovhasta) <= "15" and mes(l_gnovdesde) = mes(l_gnovhasta)   and anio(l_gnovdesde) = anio(l_gnovhasta) and  INstr ( "632,659,1640", l_rs5("estrnro")) > 0   then 
		  	  		if l_perfil <> "Administrador" then 'en las fechas entre 1 y 15 si el puesto es gerente o cajero solo pueden cargar una novedad un Administrador
				  ...
				  response.write "<script>alert('Un Empleado no tiene un Puesto.');window.close();</script>"
				  response.end
			end if 'tiene puesto
			l_rs5.close
...
' preparar parametro para invocar al noved_horarias_01 -------------------------------------------------
...
Response.write "<script>alert('Operación Realizada.');window.opener.ifrm.location = 'noved_horarias_gti_01.asp?orden='+window.opener.ifrm.datos.orden.value+'&"&l_datos&"';window.close();</script>"
