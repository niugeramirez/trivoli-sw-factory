...
if l_rep7111 = true then 

encabezado_expcasaniodestino("Total Exportado por Destino - Cereales, Aceites y Subproductos") 
...
rsOpen l_rs, cn, l_sql, 0 
if not l_rs.eof then
	l_total_toneladas = l_rs(0)
else
	l_total_toneladas = 0
end if 
l_rs.close

l_sql = " SELECT desdes, mernro, sum(conton) "
l_sql = l_sql & " FROM buq_buque "
l_sql = l_sql & " inner join buq_contenido on buq_contenido.buqnro = buq_buque.buqnro "
l_sql = l_sql & " inner join buq_destino on buq_destino.desnro = buq_contenido.desnro "
...
l_sql = l_sql & " AND buq_buque.buqfechas >= " & cambiafecha(l_anioini,"YMD",true)
l_sql = l_sql & " AND buq_buque.buqfechas <= " & cambiafecha(l_fecfin,"YMD",true)
l_sql = l_sql & " group by desdes, mernro "
rsOpen l_rs, cn, l_sql, 0 
...
do while not l_rs.eof
	...
	%>
		...
		<td align="center" width="10%" ><% if l_total_toneladas <> 0 then response.write  Round((l_totfil*100)/l_total_toneladas,2) else response.write 0 end if  %></td>		
		</tr>
	<%
	l_rs.movenext
loop
...
l_sql = " SELECT distinct(buq_sitio.sitnro), sitdes "
l_sql = l_sql & " FROM buq_buque "
l_sql = l_sql & " inner join buq_contenido on buq_contenido.buqnro = buq_buque.buqnro "
l_sql = l_sql & " inner join buq_sitio on buq_sitio.sitnro = buq_contenido.sitnro "
...
do while not l_rs.eof
	...
	'if l_rs("buqnro") <> l_buquenumero or  l_rs("sitnro") <> l_sitionumero then 
		
	
		'l_buquenumero = l_rs("buqnro")
		'l_sitionumero = l_rs("sitnro") 
	
		ArrSitMes(l_rs("sitnro") , month(l_rs("buqfechas"))  ) = ArrSitMes(l_rs("sitnro") , month(l_rs("buqfechas"))  ) + 1
	
		'if month(l_rs("buqfechas")) = 2 then 
    		'response.write "siiiiiiiii" & "<br>"
			'valor = valor + 1
			'response.write valor & "-" & l_rs("buqnro") & "<br>"
		'end if
	'else
		'if month(l_rs("buqfechas")) = 2 then 
			'response.write "noo" & "<br>"		
		'end if 			
	'end if 	
