...
<%
	...
	l_anioini = request.querystring("anioini")
	l_fecfin = request.querystring("fecfin")

	l_sql = " SELECT count(distinct(buq_agencia.agedes)) "
	...
	if not l_rs.eof then
		l_CantAgencias = l_rs(0)
	end if
	l_rs.close

	'response.write 	l_CantAgencias
	...
	do while not l_rs.eof
		arrData(l_fila,1) = l_rs(0)
		arrData(l_fila,2) = l_rs(1)
		l_fila = l_fila + 1
		l_rs.movenext
	loop
...
'response.end
	...
	strXML = "<graph caption='' subCaption='' yaxisname='Cantidad' xaxisname='Agencias' showAlternateVGridColor='1' alternateVGridAlpha='10' alternateVGridColor='AFD8F8' numDivLines='4' decimalPrecision='0' canvasBorderThickness='1' canvasBorderColor='114B78' baseFontColor='114B78' hoverCapBorderColor='114B78' hoverCapBgColor='E7EFF6'> "
