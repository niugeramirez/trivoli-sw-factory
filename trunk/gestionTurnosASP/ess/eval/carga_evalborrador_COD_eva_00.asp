...
<% 
'=====================================================================================
'Archivo  : carga_evalborrador_COD_eva_00.asp
'Objetivo : Carga Evaluacion Borrador
'Fecha	  : 10-02-2005
'Autor	  : CCRossi
...
do while not l_rs.eof 
	...
	if l_rs2.eof then
		...
		l_sql = l_sql & " values (" & l_evldrnro & "," & l_rs("evaobjnro") &",-1)"
...
if l_mostrar=0 then%>
	<tr>
        <th align=center colspan="<%=l_col%>"> - NO HABILITADO -</th>
    </tr>
<%else
...
l_sql = l_sql & " WHERE evaluaobj.evldrnro	  = " & l_evldrnro
l_sql = l_sql & "   AND evaluaobj.evaborrador = -1 "
l_sql = l_sql & " ORDER BY evatipoobj.evatipobjorden "
rsOpen l_rs, cn, l_sql, 0 
l_evatipobjnro=""
l_cantidad = 0
...
do until l_rs.eof
    ...
    <tr>
        ...
        <td align=center>
			<input type="text" name="evaobjalcanza<%=l_rs("evaobjnro")%>" size=3 maxlength=3 value="<%=PasarComaAPunto(l_rs("evaobjalcanza"))%>">
			&nbsp;<a href=# class="linkgrande" alt="Notas Posibles" onclick="alert(resultados);">?</a>
		</td>
        <td align=center>
        ...
        <td valign=top>
			<a href=# onclick="if (Controlar(document.datos.evaobjalcanza<%=l_rs("evaobjnro")%>,document.datos.evasgtotexto<%=l_rs("evaobjnro")%>)) { if (ValidarDatos(document.datos.evaobjalcanza<%=l_rs("evaobjnro")%>)) {grabar.location='grabar_evalborrador_COD_eva_00.asp?evldrnro=<%=l_evldrnro%>&evaobjnro=<%=l_rs("evaobjnro")%>&evaobjalcanza='+document.datos.evaobjalcanza<%=l_rs("evaobjnro")%>.value+'&evasgtotexto='+escape(Blanquear(document.datos.evasgtotexto<%=l_rs("evaobjnro")%>.value));document.datos.grabado<%=l_rs("evaobjnro")%>.value='M'; } }">Grabar</a>
			<br>
...
<%
	...
	if not l_rs.eof then
		...
		<td align=center>
			...
			&nbsp;<a href=# alt="Notas Posibles" class="linkgrande" onclick="alert(notafinal);">?</a>
		</td>
		...
		<td align=center>
			...
			&nbsp;
			<a href=# alt="Notas Finales" class="linkgrande" onclick="alert(notafinal);">?</a>
		</td>
		<td><div id="correspondencia<%=l_evatipobjnro%>"></div></td>
		<td>&nbsp;</td>
		</tr>
