...
<% 
' Modificado: 20-01-2004- CCRossi- Agregar antiguedad en puesto,puesto,genrencia,edad
'			  21-01-2004- CCRossi- Agregar antiguedad en empresa
'			  27-10-2004- CCRossi- Cambiar la tabla para quye no se mueva al dar click
'Modif.	  : CCRossi - 03-11-2004 - LABEL Revisor si no es ABN sino Evaluador
'Modif.	  : CCRossi - 22-11-2004 - LABEL Pisicon si  es ABN sino Puesto
'---------------------------------------------------------------------------------------
...
'   selecciono  -------- 
Set rs9 = Server.CreateObject("ADODB.RecordSet")
l_sql = "SELECT   evaengdesabr, evaengcodext, evaclinom, evaclicodext "
...
if (leg != "")
	{
	document.location ="form_carga_eva_00.asp?evaevenro=<%= l_evaevenro %>&empleg=" + leg + "&revisor=" + document.datos.rempleg.value+ "&pantalla=<%=l_pantalla%>" ;
	}
}
...
l_sql = l_sql & " FROM his_estructura "
l_sql = l_sql & " INNER JOIN estructura ON estructura.estrnro=his_estructura.estrnro "
l_sql = l_sql & "		 AND estructura.tenro = 4 " 
l_sql = l_sql & " WHERE his_estructura.ternro=" & l_ternro
l_sql = l_sql & " AND   his_estructura.htethasta IS NULL " 
l_sql = l_sql & " ORDER BY his_estructura.htetdesde DESC " 
rsOpen l_rs, cn, l_sql, 0
...
l_antiguedadpuesto = l_anios &" años "& l_meses &" meses "& l_dias &" dias "
end if
...
function borrar(){
 if (document.all.btnborrar.className == "sidebtnABM"){
	if (confirm('¿Borra la evaluación?'))
...
<tr >
       	<td align="left" class="barra">Formulario de Carga de : <%= l_evaevedesabr %></td>
      	<td align="right" class="barra" colspan=2 valign="middle">
      		<%if trim(l_evacabnro)<>"" and cejemplo<>-1 and cdeloitte<>-1 then%>
...
<% if cdeloitte=-1 then 
		' lo saque - ver si lo llama alguien				%>
<% else %>
<% end if %>
<tr height="5%">
	<td width="65%">
		...
		<tr>
			...
			<td >
				<a href="JavaScript:Sig_Ant(<%= anterior %>)"><img align="absmiddle" src="/serviciolocal/shared/images/prev.jpg" alt="Legajo Anterior (<%= anterior %>)" border="0"></a>
				...
				<a href="JavaScript:Sig_Ant(<%= siguiente %>)"><img align="absmiddle" src="/serviciolocal/shared/images/next.jpg" alt="Legajo Siguiente (<%= siguiente %>)" border="0"></a>
		...
		<tr>
			<td colspan="2"><font <%=l_letra%>><b>Gerencia:</b>&nbsp;<input class="blanc" type="Text" name="gerencia" size="30" value="<%=l_gerencia%>">
		    &nbsp;<font <%=l_letra%>><b><%if cejemplo=-1 then%>&nbsp;<%else%>Puesto:<%end if%></b>&nbsp;<%if cejemplo=-1 then%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%else%><input class="blanc" type="Text" name="puesto" size="30" value="<%=l_puesto%>"><%end if%>
		    &nbsp;<font <%=l_letra%>><b>Edad:</b>&nbsp;<input class="blanc" type="Text" name="edad" size="5" value="<%=l_edad%>">
		    </td>
		</tr>
		<tr>
			<td colspan="2">
				...
				Set cn = Nothing%>  
				<input class="blanc" type="Text" name="antiguedad" size="25" value='<%=l_texto%>'>
				&nbsp;<font <%=l_letra%>><b><%if cejemplo=-1 then%>&nbsp;&nbsp;&nbsp;<%else%>en el Puesto:<%end if%></b>&nbsp;
				<%if cejemplo=-1 then%>&nbsp;&nbsp;&nbsp;<%else%><input class="blanc" type="Text" name="antiguedadpuesto" value="<%=l_antiguedadpuesto%>" size="25"><%end if%>
			</td>
		</tr>
		</table> 
	</td>
	<td width="10">
		<a name="btnagregar" <%if l_yaestaba then %>class=sidebtnDSB <%else%> class=sidebtnABM <%end if%> href="Javascript:agregar();"><b>Agregar>></b></a>
		<br>
		<a name="btnborrar" <%if l_yaestaba then %>class=sidebtnABM <%else%> class=sidebtnDSB <%end if%> href="Javascript:borrar();window.location.reload();"><b>&nbsp;&nbsp;&nbsp;&nbsp;Borrar&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b></a>
	</td>
	<td>
		<table border="0" cellpadding="0" cellspacing="0">
		<tr>
		    <td align="right"><font <%=l_letra%>><b>Per&iacute;odo Desde:</b></td>
			<td><input <%=l_letra%> class="blanc" type="Text" name="usr" size="10" value="<%= l_evaperdesde %>"></td>
		</tr>	
		<tr>	
		    <td align="right" ><font <%=l_letra%>><b>Hasta:</b></td>
			<td><input <%=l_letra%> class="blanc" type="Text" name="usr" size="10" value="<%= l_evaperhasta %>"></td>
		</tr>
		</table> 
	</td>
</tr>
...
<tr height="5%">
	<td colspan="3">
		...
		<tr>
		    <td align="right"><font <%=l_letra%>><b><%if cejemplo=-1 then%>Evaluador:<%else%>Revisor:<%end if%></b></td>
			<td >
				<font <%=l_letra%>>
				...
				&nbsp;
				<font <%=l_letra%>><b><%if cejemplo=-1 then%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%else%>Puesto:<%end if%></b>&nbsp;<%if cejemplo=-1 then%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%else%><input disabled class="blanc" type="Text" name="evalpuesto" size="25" value="<%=l_evalpuesto%>"><%end if%>
			</td>
			<td>
				<font <%=l_letra%>><b>Evaluaci&oacute;n Aprobada:</b>&nbsp;
