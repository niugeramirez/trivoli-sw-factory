...
<% 
'=====================================================================================
'Archivo  : carga_cierreEva_COD_eva_00.asp
...
'buscar la evacab
 ...
 l_sql = "SELECT evacabnro, evatevnro FROM  evadetevldor INNER JOIN empleado ON empleado.ternro = evadetevldor.evaluador "
...
{
	if (ver=='0')
	 document.ifrmgarante.location = 'carga_cierreEva_COD_eva_01.asp?evldrnro=<%=l_evldrnro%>&evaseccnro=<%=l_evaseccnro%>&mostrar=1';
	else
	 document.ifrmgarante.location = 'carga_cierreEva_COD_eva_01.asp?evldrnro=<%=l_evldrnro%>&evaseccnro=<%=l_evaseccnro%>&mostrar=0';		
}
</script>
...
do while not l_rs1.eof
	...
	if l_primero=-1 then%>
		<tr>
		 	...
		 	<td align=left>
		 		<input type="hidden" name="evaacuerdo" value="<%=l_rs1("evaacuerdo")%>">
				<%IF l_evatevnro=cgarante then%>
		 		<input type="Radio" readonly disabled onclick="document.datos.evaacuerdo.value=0;garante('0');" name="radio" value="0" <%If l_rs1("evaacuerdo") = 0 then%>checked<%End If%>>Desacuerdo
		 		<input type="Radio" readonly disabled onclick="document.datos.evaacuerdo.value=-1;garante('1');" name="radio" value="-1" <%If l_rs1("evaacuerdo") = -1 then%>checked<%End If%>>Acuerdo
				<%else%>
				<input type="Radio" onclick="document.datos.evaacuerdo.value=0;garante('0');" name="radio" value="0" <%If l_rs1("evaacuerdo") = 0 then%>checked<%End If%>>Desacuerdo
		 		<input type="Radio" onclick="document.datos.evaacuerdo.value=-1;garante('1');" name="radio" value="-1" <%If l_rs1("evaacuerdo") = -1 then%>checked<%End If%>>Acuerdo
				<%end if%>
		 	</td>
		</tr>
		...
		<tr>
		 	...
		 	<td align=left>
		 		<input type="hidden" name="evareunion" value="<%=l_rs1("evareunion")%>">
		 		<input type="Radio" onclick="document.datos.evareunion.value=0;" name="reunion" value="0" <%If l_rs1("evareunion") = 0 then%>checked<%End If%>>NO
		 		<input type="Radio" onclick="document.datos.evareunion.value=-1;" name="reunion" value="-1" <%If l_rs1("evareunion") = -1 then%>checked<%End If%>>SI
		 	</td>
		</tr>
	<%
	l_primero=0
...
<tr>
	...
	<%if l_evatevnro <> cautoevaluador and l_evatevnro<> cevaluador then%>
		<a href=# class="linkgrande" onclick="if (Controlar(document.ifrmgarante.datos.evaobser<%=l_evldrnro%>)) {grabar.location='grabar_cierre_COD_eva_00.asp?evldrnro=<%=l_evldrnro%>&evaetapa=<%=l_evaetapa%>&evaobser='+escape(Blanquear(document.ifrmgarante.datos.evaobser<%=l_evldrnro%>.value))+'&evafecha='+document.datos.evafecha.value+'&evareunion='+document.datos.evareunion.value+'&evaacuerdo='+document.datos.evaacuerdo.value+'&notafinalpropuesta='+document.datos.notafinalpropuesta.value+'&notafinal='+document.ifrmgarante.datos.notafinal.value;document.datos.grabado.value='G';}">GRABAR</a>
	<%else%>
		<a href=# class="linkgrande" onclick="if (Controlar(document.datos.evaobser<%=l_evldrnro%>)) {grabar.location='grabar_cierre_COD_eva_00.asp?evldrnro=<%=l_evldrnro%>&evaetapa=<%=l_evaetapa%>&evaobser='+escape(Blanquear(document.datos.evaobser<%=l_evldrnro%>.value))+'&notafinalpropuesta='+document.datos.notafinalpropuesta.value+'&evareunion='+document.datos.evareunion.value+'&evaacuerdo='+document.datos.evaacuerdo.value+'&evafecha='+document.datos.evafecha.value;document.datos.grabado.value='G';}">GRABAR</a>
	<%end if%>		
