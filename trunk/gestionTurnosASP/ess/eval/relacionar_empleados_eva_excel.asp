...
<% Response.AddHeader "Content-Disposition", "attachment;filename=relacionar.xls" %>
<% Server.ScriptTimeout = 720 %>
<%
'---------------------------------------------------------------------------------
'Archivo	: relacionar_empleados_eva_excel.asp
...
<table id="tabla" name="tabla">
    <tr>
        <th colspan="4"><%if ccodelco=-1 then%>Supervisados<%else%>Empleado<%end if%>&nbsp;del Evento</th>
    </tr>
    <tr>
