...
if l_empleg = "" then
	...
	l_sql = "SELECT min(empleg) FROM v_empleado "
	l_sql = l_sql & " INNER JOIN evacab ON (v_empleado.ternro=evacab.empleado and evacab.evaevenro="& l_evaevenro &")"
	RS9.Maxrecords = 1
...
Set l_rs = Server.CreateObject("ADODB.RecordSet")
l_sql = "SELECT v_empleado.terape, v_empleado.ternom, v_empleado.empreporta, v_empleado.ternro, v_empleado.empfoto, terfecnac "
l_sql = l_sql & " FROM v_empleado "
l_sql = l_sql & " INNER JOIN tercero ON tercero.ternro= v_empleado.ternro "
...
if clitoral = -1 then
	l_sql = l_sql & " AND   his_estructura.tenro = 54 " 
else
...
' si no tiene empreporta busco el evaluador
	...
	l_sql = l_sql & " inner join v_empleado revisor on revisor.ternro= evadetevldor.evaluador "
