...
' Response.Write l_empleg & "<br>" & l_evaluador
 if trim(l_empleg)<>"" and not isNull(l_empleg) then
	if (trim(l_empleg) = trim(l_evaluador)) or (l_evatevnro= cgarante) then
		l_mostrar = "1"
	else	
		l_mostrar = "0"
	end if
...
do until l_rs.eof
    ...
    <tr>
        ...
        <td align=center>
			...
			&nbsp;<a href=# class="linkgrande" onclick="alert(resultados);">?</a>
		</td>
        <td align=center>
...
<%
	...
	if not l_rs.eof then
		...
		<td align=center>
			...
			&nbsp;<a href=# class="linkgrande" onclick="alert(notafinal);">?</a>
		</td>
