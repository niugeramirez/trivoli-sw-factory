...
' de uso local  
 ...
 ' dim l_areatrnro
  dim l_evaareadesc
  dim l_descrip ' descripc de la competencia
...
' __________________________________________________________________________
sub descripComp (evafacnro, evacabnro ,descComp, descrip)',evatevnro
descrip=""
...
descComp = "<input type=""Text"" name=""resudesc"&evafacnro&l_evldrnro &""" value="""& descrip&""" size=""60"" maxlength=""150"" disabled>"
end sub
...
<tr>
	<th nowrap class="th2">AREAS/DESCRIPCI&Oacute;N COMPETENCIAS </th>
	<% For i = 1 to l_cantevldores %>
...
'response.write l_sql
 ...
 do while not l_rs.eof 
	...
	if trim(l_evatitdesabr) <> trim(l_rs("evatitdesabr")) then %>
		<tr style="height:20">
			<td align=left valign="middle" colspan="2"><b>AREA:</b> <%=l_rs("evatitdesabr")%> &nbsp;<br> &nbsp;</td>	
			<td>&nbsp; </td>
		<% For i= 1 to l_cantevldores 
...
<%			l_evatitdesabr = l_rs("evatitdesabr") 
	...
	<% descripComp l_rs("evafacnro"), l_evacabnro,l_compDescrip, l_descrip %>
	<% if l_descrip <> "" then %>
	<tr>
...
<iframe src="blanc.asp" name="terminarsecc" src="termsecc_areasyresultadosSE_eva_00.asp?tipo=eval&evacabnro=<%=l_evacabnro%>&evaseccnro=<%=l_evaseccnro%>&evldrnro=<%=l_evldrnro%>&evatevnro=<%=l_evatevnro%>" style="visibility:hidden;width:0;height:0">
</iframe>
</form>	
</body>
</html>
