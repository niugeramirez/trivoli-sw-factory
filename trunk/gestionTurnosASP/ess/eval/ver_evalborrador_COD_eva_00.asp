...
function Controlar(alcanza,texto){
	...
	{
		<%if cint(l_haycompromisos)=1 then%>
		Sumar();
		Puntuacion();
		<%end if%>
		return true;		
...
<body leftmargin="0" rightmargin="0" topmargin="0" bottommargin="0" onload="<%if cint(l_haycompromisos)=1 and l_mostrar="1" then%>;Sumar();Puntuacion()<%end if%>;">
<table>
    <tr>
		<th class="th2">&nbsp;</th>
...
do until l_rs.eof
    ...
    <tr>
        ...
        <td align=center>
			<input style="background : #e0e0de;" readonly type="text" class="blanc" name="evaobjalcanza<%=l_rs("evaobjnro")%>" size=3 value="<%=PasarComaAPunto(l_rs("evaobjalcanza"))%>">
			&nbsp;<a href=# onclick="alert(resultados);">?</a>
		</td>
        <td align=center>
			<input style="background : #e0e0de;" readonly type="text" class="rev" name="puntacion<%=l_rs("evaobjnro")%>" size=4 >
			&nbsp;&nbsp;
		</td>
        <td align=center>
			<textarea style="background : #e0e0de;" readonly type="text" class="rev" name="evasgtotexto<%=l_rs("evaobjnro")%>" cols=20 rows=4><%=trim(l_rs("evasgtotexto"))%></textarea>
		</td>
        <td valign=top>
