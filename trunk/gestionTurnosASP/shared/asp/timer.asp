...
If (Session("Time") + Plazo < now) and (UCase(Session("Username")) <> "SUPER") then
	...
	if request("menu") = "no" then
		Response.write "<script>window.parent.location = '/serviciolocal/lanzador/lanzador2.asp';</script>"
	else
		'Response.write "<script>window.parent.parent.opener.location = '/serviciolocal/lanzador/lanzador2.asp';</script>"
		Response.write "<script>window.parent.parent.opener.location.reload();</script>"
	end if
end if
%>	
