<% Option Explicit %>
<!--
Archivo: filtro_doblebrowse.asp
Descripción: Filtra items en el doble browse
Autor: F. Favre  
Fecha: 10-03
Modificado:
-->
...
function filtrar(){
	for (i=0;i<=<%= l_cantidad %>;i++){
  	...
  	if (sel.substr(0,1) == "R"){       		// Restaurar
		window.opener.LimpiarFiltro(<%= l_lado %>);
    	window.opener.CargarLado(<%= l_lado %>); 
		window.close();  
    }
  	else{
		if (sel.substr(0,1) == "T")    		// Es un texto 
	  		window.location = "filtro_doblebrowse_txt.asp?campo=" + sel.substr(1) + "&lado=<%= l_lado %>"
		else{
 	  		if (sel.substr(0,1) == "N") 	// Es numerico 
  	  			window.location = "filtro_doblebrowse_num.asp?campo=" + sel.substr(1) + "&lado=<%= l_lado %>"
			else{
	 	  		if (sel.substr(0,1) == "B") 	// Es Boolean 
	  	  			window.location = "filtro_doblebrowse_bool.asp?campo=" + sel.substr(1) + "&lado=<%= l_lado %>"
				else							// Es fecha
	  	    		window.location = "filtro_doblebrowse_fec.asp?campo=" + sel.substr(1) + "&lado=<%= l_lado %>"
			}
		}
    }
}
