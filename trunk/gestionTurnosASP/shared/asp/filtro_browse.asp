...
function filtrar()
{
  ...
  if (sel.substr(0,1) == "R") // Restaurar
	...
	{
	  window.location = "filtro_browse_txt.asp?campo=" + sel.substr(1) + "&fuente=<%= l_destino %>&orden=<%= l_orden %>"
	}  
	else
 	  if (sel.substr(0,1) == "N") // Es numerico 
  	    window.location = "filtro_browse_num.asp?campo=" + sel.substr(1) + "&fuente=<%= l_destino %>&orden=<%= l_orden %>"
	  else	// Es fecha
  	    window.location = "filtro_browse_fec.asp?campo=" + sel.substr(1) + "&fuente=<%= l_destino %>&orden=<%= l_orden %>"
    }
}
