...
l_param = Request.querystring("param")
...
var filtro="";
  ...
  if (sel.substr(0,1) == "R") // Restaurar
	...
	{
	  filtro = window.showModalDialog("dial_filtro_browse_txt.asp?campo=" + sel.substr(1) + "&fuente=<%= l_destino %>&orden=<%= l_orden %>",'','dialogWidth:20;dialogHeight:12.2');
	}  
	else
...
window.opener.ifrm.location = '<%= l_destino %>?orden=<%= l_orden %>&filtro='+filtro+'&<%=l_param%>'; 
window.close();  
