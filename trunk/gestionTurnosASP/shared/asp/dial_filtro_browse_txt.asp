<% Option Explicit %>
<%
' 07-12-04 - Fernando Favre - Se valida que el texto contenga caracteres validos.
Dim l_campo 
Dim l_fuente 
Dim l_orden 
...
function filtrar()
{
  ...
  if (sel == "1")	
    var txt = "<%= l_campo %> LIKE '" + document.datos.texto.value + escape("%") + "'"
  if (sel == "2")	
    var txt = "<%= l_campo %> LIKE '" + escape("%") + document.datos.texto.value + escape("%") + "'"
  if (sel == "3")	
...
<tr>
    <td align="right" class="th2">
		...
		<a class=sidebtnABM hhref="#" onclick="Javascript:window.close()">Cancelar</a>		
	</td>
</tr>
</table>
</form>
